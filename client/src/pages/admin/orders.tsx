...
import { Eye, Search, Filter, Download, Trash2, MoreHorizontal, CheckSquare, XSquare } from "lucide-react";
...
export default function AdminOrders() {
  ...
  const handleBulkStatusUpdate = async (newStatus: string) => {
    ...
    try {
      const updates = Array.from(selectedOrderIds).map(id => {
        const orderRef = ref(firebaseDatabase, `orders/${id}`);
      ...
      toast({
        title: "Success",
        description: `${selectedOrderIds.size} orders updated to ${newStatus}`,
      });
  ...
  const handleBulkDelete = async () => {
    ...
    if (!confirm(`Are you sure you want to delete ${selectedOrderIds.size} orders? This cannot be undone.`)) {
      return;
    }

    try {
      const deletions = Array.from(selectedOrderIds).map(id => {
        const orderRef = ref(firebaseDatabase, `orders/${id}`);
      ...
      toast({
        title: "Success",
        description: `${selectedOrderIds.size} orders deleted successfully`,
      });
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="sm" className="flex items-center gap-2 h-8">
                  ...
                  Bulk Actions ({selectedOrderIds.size})
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-56">
                ...
                <DropdownMenuItem onClick={() => handleBulkStatusUpdate("cancelled")} className="text-red-600 focus:text-red-600">
                  Mark as Cancelled
                </DropdownMenuItem>
                <DropdownMenuSeparator />
