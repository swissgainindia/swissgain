...
const getAffiliateId = (): string => {
  // Get UID from your auth system cookie
...
export default function AffiliateSales() {
  ...
  const loadTotalWithdrawn = () => {
    try {
      const withdrawnRef = ref(database, `affiliates/${affiliateId}/totalWithdrawn`);
      
      const unsubscribe = onValue(withdrawnRef, (snapshot) => {
        if (snapshot.exists()) {
          setTotalWithdrawn(snapshot.val() || 0);
        } else {
          setTotalWithdrawn(0);
        }
      });
      ...
      const unsubscribe = onValue(affiliateRef, (snapshot) => {
        if (snapshot.exists()) {
          ...
          Object.entries(productsData).forEach(([productId, value]: [string, any]) => {
            // Only include products with matching affiliateId
            if (value.affiliateId === affiliateId) {
              productsArray.push({
                productId: productId,
                affiliateId: value.affiliateId,
...
<div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
  <div>
    ...
    <p className="text-muted-foreground text-sm md:text-base">
      Track your sales and products performance - Only showing data for your affiliate ID
    </p>
  </div>
  ...
  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
    <div className="flex items-start md:items-center space-x-3">
      ...
      <div>
        ...
        <p className="text-green-600 text-sm">
          This dashboard shows data only for your unique affiliate ID
        </p>
        <div className="flex flex-wrap items-center mt-1 text-xs text-green-500">
          <span className="w-2 h-2 rounded-full bg-green-500 mr-2"></span>
          Personal Data • {sales.length} Sales • {affiliateProducts.length} Products
        </div>
      </div>
    </div>
    ...
    <div className="text-left md:text-right">
      ...
      <div className="flex items-center">
        <code className="bg-white px-3 py-1 rounded text-green-800 font-mono text-sm border border-green-200 max-w-[200px] truncate">
          {affiliateId}
        </code>
        <Button
          variant="outline"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {activeTab === "sales" ? "Your personal orders" : "Products you promoted"}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {activeTab === "sales" ? "Generated through your links" : "Your link clicks"}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {activeTab === "sales" ? "Your personal earnings" : "Your product sales"}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {activeTab === "sales" ? "Your pending orders" : "Your product commission"}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {activeTab === "sales" ? "Your delivered orders" : "Your products with sales"}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Your Commission Balance</CardTitle>
          <CardDescription>Overview of your total earnings and available amount after withdrawals</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center p-3 bg-destructive/5 rounded-lg border border-destructive/10">
              <div>
                <p className="text-sm text-destructive">Total Withdrawn</p>
                <p className="text-lg font-bold text-destructive">₹{totalWithdrawn.toLocaleString()}</p>
              </div>
            ...
            <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg border border-green-200">
              <div>
                ...
                <p className="text-xl font-bold text-green-600">₹{stats.pendingCommission.toLocaleString()}</p>
              </div>
              <DollarSign className="h-5 w-5 text-green-600" />
            </div>
          </div>
            ...
            {filteredSales.length === 0 ? (
              <div className="text-center py-12">
                ...
                <div className="mt-4 text-sm text-muted-foreground">
                  Your Affiliate ID: <code className="bg-muted px-2 py-1 rounded">{affiliateId}</code>
                </div>
              </div>
            ) : (
      ...
      <Card>
        <CardHeader>
          <CardTitle>How Your Personal Affiliate System Works</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                ...
                tracked across all your promotions and sales. Only your data is shown here.
              </p>
            </div>
            <div className="space-y-2">
              <h4 className="font-semibold">Personal Performance</h4>
              <p className="text-sm text-muted-foreground">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                Earn ₹100 commission for every product sold through your links. 
                Track your personal earnings and get paid monthly.
              </p>
            </div>
          </div>
        </CardContent>
