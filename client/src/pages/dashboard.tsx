...
export default function Dashboard() {
  ...
  // Mobile scrollable navigation items
  const mobileNavItems = [
    { id: 'overview', label: 'Overview', icon: <Home className="h-5 w-5" /> },
    { id: 'affiliate-earnings', label: 'Affiliate', icon: <BarChart3 className="h-5 w-5" /> },
    { id: 'sales-dashboard', label: 'Sales', icon: <TrendingUp className="h-5 w-5" /> },
    { id: 'referral-earnings', label: 'Referrals', icon: <User className="h-5 w-5" /> },
    { id: 'profile', label: 'Profile', icon: <User className="h-5 w-5" /> },
    { id: 'billing', label: 'Billing', icon: <CreditCard className="h-5 w-5" /> },
    { id: 'payment-request', label: 'Payment', icon: <CreditCard className="h-5 w-5" /> },
    { id: 'support-center', label: 'Support', icon: <Headphones className="h-5 w-5" /> },
  ...
  const PurchaseRequiredMessage = () => (
    <div className="flex flex-col items-center justify-center min-h-[400px] text-center p-4 md:p-8">
      <div className="w-16 h-16 md:w-20 md:h-20 bg-amber-100 rounded-full flex items-center justify-center mb-4 md:mb-6">
        <ShoppingBag className="h-8 w-8 md:h-10 md:w-10 text-amber-600" />
      </div>
      ...
      <div className="bg-amber-50 p-4 rounded-lg border border-amber-200 mb-4 md:mb-6 max-w-md w-full">
        <div className="flex items-start">
          <AlertCircle className="h-4 w-4 md:h-5 md:w-5 text-amber-600 mr-2 mt-0.5 flex-shrink-0" />
      ...
      <div className="flex flex-col sm:flex-row gap-3 md:gap-4 w-full max-w-md">
        <Button onClick={() => setLocation('/products')} className="gradient-primary w-full sm:w-auto">
    ...
    return (
      <div className="min-h-screen bg-background pb-16 md:pb-0">
        <div className="max-w-7xl mx-auto px-4 py-4 md:py-8">
          <header className="bg-background border-b px-4 md:px-6 py-3 md:py-4 mb-6 md:mb-8">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-lg md:text-2xl font-bold">SwissGain Dashboard</h1>
  ...
  return (
    <div className="flex bg-background min-h-screen md:my-5 md:border relative">
      {/* Mobile Menu Overlay */}
      {mobileMenuOpen && (
        <div 
      ...
      <div className={`
        ...
        transition-transform duration-300 w-64 flex flex-col border-r
        md:hidden transform ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}
      `}>
        ...
        <div className="flex-1 py-4 overflow-y-auto">
          <nav className="space-y-1 px-3">
            ...
            <SidebarItem
              ...
              onClick={() => {
                setActiveTab('affiliate-earnings');
                setMobileMenuOpen(false);
              }}
              collapsed={false}
            />
            ...
            <SidebarItem
              ...
              onClick={() => {
                setActiveTab('referral-earnings');
                setMobileMenuOpen(false);
              }}
              collapsed={false}
            />
      ...
      {/* Desktop Sidebar */}
      <div className={`
        hidden md:flex bg-sidebar text-sidebar-foreground transition-all duration-300 
        ${sidebarCollapsed ? 'w-16' : 'w-64'} flex flex-col border-r
      `}>
        <div className="p-4 border-b border-sidebar-border">
          <div className="flex items-center justify-between">
            {!sidebarCollapsed && (
      ...
      <div className="flex-1 flex flex-col w-full">
        ...
        <header className="bg-background border-b px-4 md:px-6 py-3 md:py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3 md:space-x-4">
              ...
              <div>
                <h1 className="text-lg md:text-2xl font-bold capitalize">{activeTab.replace('-', ' ')}</h1>
                <p className="text-muted-foreground text-sm md:text-base">
                  Welcome back, {dashboardUserData.name}!
                  {hasPurchased && (
            ...
            <div className="flex items-center space-x-2 md:space-x-4">
              {hasPurchased && (
                <Badge variant="outline" className="md:hidden bg-green-50 text-green-700 border-green-200 text-xs">
                  ✓
                </Badge>
              )}
              <Button variant="outline" size="sm" onClick={handleLogout} className="hidden md:inline-flex">
        ...
        {/* Content Area */}
        <main className="flex-1 p-3 md:p-6 overflow-auto pb-20 md:pb-0">
          {renderContent()}
        </main>
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-background border-t border-border z-30">
          <div className="relative">
            <div className="overflow-x-auto scrollbar-hide">
              <div className="flex items-center justify-start min-w-max h-16 px-2 space-x-1">
                {mobileNavItems.map((item) => (
                  <button
                    ...
                    className={`
                      ...
                      w-16 h-16 px-1 transition-colors duration-200
                      ${activeTab === item.id 
                        ? 'text-primary' 
      ...
      <Dialog open={showAffiliateLinkDialog} onOpenChange={setShowAffiliateLinkDialog}>
        <DialogContent className="max-w-md mx-auto">
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            <Button variant="outline" onClick={() => setShowAffiliateLinkDialog(false)} className="w-full sm:w-auto">
              Cancel
            </Button>
            <Button onClick={handleGenerateAffiliateLink} className="w-full sm:w-auto">
              Generate Link
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={showReferralLinkDialog} onOpenChange={setShowReferralLinkDialog}>
        <DialogContent className="max-w-md mx-auto">
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            <Button variant="outline" onClick={() => setShowReferralLinkDialog(false)} className="w-full sm:w-auto">
              Cancel
            </Button>
            <Button onClick={handleGenerateReferralLink} className="w-full sm:w-auto">
              Generate Link
            </Button>
          </DialogFooter>
        </DialogContent>
